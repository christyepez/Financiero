...
No SRI API behavior changes were introduced in Sprint 11 P3. External SQL/Portal evidence remains `NoResponse` / `EvidencePending`, preflight returned `SCRIPT_EXIT=2`, and productive tax flows remain blocked until accepted evidence plus `SCRIPT_EXIT=0`.
...
## Sprint 10 P3 external dependency note

P3 adds no SRI API endpoints and no production activation. External SQL/Portal evidence remains `EvidencePending` / `BLOCKED_DEPENDENCY`; therefore SRI Test real, SRI Production, official ATS, legal-final RIDE and productive XAdES remain blocked.
