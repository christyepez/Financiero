  ...
  template: `
    ...
    @if (items().length) {
      <section class="panel">
        ...
        <table class="table">
          <thead><tr><th>Tipo</th><th>Secuencial</th><th>Período</th><th>Estado</th></tr></thead>
          <tbody>
            @for (item of items(); track item.id) {
              <tr>
                <td>{{ item.documentType || '-' }}</td>
                <td>{{ item.establishment || '000' }}-{{ item.emissionPoint || '000' }}-{{ item.sequential || '000000000' }}</td>
      ...
      <fin-empty-state title="Sin anulados para el período" description="Vista read-only. No registra anulaciones desde Angular en P1." />
    }
  `
})
